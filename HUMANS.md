...
- **Remix text**: select any passage (even in core content) → ✨ in the popup (or right-click) → describe the change → a **preview** appears with the changed span highlighted amber. Then you decide: **✓ Accept** makes it your version of that section (it replaces the original *in your book* — highlights stay, one-click revert anytime), **✓ Accept & share** also sends it to other readers and the editors (your prompt travels with it, so editors see exactly what you wanted changed), **✗ Discard** throws it away. The book's original is never touched.
